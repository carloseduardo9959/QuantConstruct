...
{
    public partial class frmChapisco : Form
    {
        public frmChapisco()
        {
            InitializeComponent();
        ...
        private void btnCalcular_Click(object sender, EventArgs e)
        {
            ...
            double chapisco = (double.Parse(txtQtdLado.Text) * double.Parse(txtComprimento.Text) * double.Parse(txtAltura.Text));

            double argamassa = (chapisco - (areaJanela + areaPorta)) * (double.Parse(txtEspessura.Text)/1000);
            double area = chapisco - (areaJanela + areaPorta);

            lblArgamassa.Text = argamassa.ToString("0.000");
            lblArea.Text = area.ToString("0.000");

            double cimento = ((area) / (11 * 50 / 21.6));
            double areiaLT = cimento * 3 * (50 / 21.6);
            double areiaMT = areiaLT * 0.018;
            double aguaLT = cimento * 0.5 * (50 / 21.6);
            double aguaLitro = aguaLT * 18;
            double aditivo = ((cimento * 0.25) * 18) * (50 / 21.6);
